...
class Solver:
    ...
    def getBm(self,rp:np.ndarray,p:np.ndarray,noised=True):
        ...
        if noised:
            if self.paras.intrisicNoise < 1e-20:
...
class Visualizer:
    ...
    def showQ(self,solver:Solver,Q:np.ndarray,ax:vs.plt.Axes):
        if solver.paras.dim == 2:
            ps = solver.sourcePoints*1e2
            amplitude = Q**2/np.max(Q**2)
            ax.scatter(
                ps[0,:],ps[2,:],s=30,c=amplitude,cmap="Reds"
            )
...
class BiVarContraller: # 双变量 
    ...
    def run(self,saveFolder=""):
        ...
        for k1,k2 in tqdm.tqdm(list(itertools.product(range(self.xs1.size),range(self.xs2.size))),desc=f"{self.baseParas.getLabel()}",position=processIndex):
            x1 = self.xs1[k1]
            x2 = self.xs2[k2]
